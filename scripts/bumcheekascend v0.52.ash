...
if (borisOptions != "") borisOptions += "0"; // if we run out of options, default to manual stop 
...
void bcAutoBoris() {
	int i, borisOption, borisLength;
	string borisResult;

	if (borisOptions != "") {
	borisLength = length(borisOptions);
		for i from 0 to borislength - 1 {
			...
			while (contains_text(borisResult, "hungering for knowledge") && contains_text(borisResult, borisTree[borisOption])) {
				if (borisOption == 0) abort("BCC: stopping so you can worship in the glory of Boris."); 
				print("BCC: "+borisTree[borisOption], "purple");
				borisResult = visit_url("da.php?whichtree="+borisOption+"&action=borisskill");
			}
		}
	} else {
		print("BCC: Not worshipping in glory of Boris","purple");
	}
}
...
boolean buMax(string maxme, int maxMainstat) {
	...
	if (contains_text(maxme, "nuns")) {
		...
		switch (my_primestat()) {
			case $stat[Muscle] : 		cli_execute("maximize "+max_bees+" beeosity, " + (my_path() == "Zombie Slayer" ? "" : "0.5 mp regen max") + ", mainstat " + ((bcasc_AllowML) ? "" : "-10 ml") + " +melee "+((anHero()) ? "+shield" : "")+" +outfit "+bcasc_warOutfit); break;
			...
			case $stat[Moxie] : 		cli_execute("maximize "+max_bees+" beeosity, 0.5 mp regen max, mainstat " + ((bcasc_AllowML) ? "" : "-10 ml") + " -melee +outfit "+bcasc_warOutfit); break;
		}
		string sns = prepSNS();
		if (sns != "") {
	...
	switch (my_primestat()) {
		case $stat[Muscle] : 		cli_execute("maximize "+max_bees+" beeosity, mainstat "+maxMainstat+" max, "+maxme+" +melee "+((anHero()) ? "+shield" : "") + ((bcasc_AllowML) ? "" : " -10 ml") + " +muscle experience " + (my_path() == "Zombie Slayer" ? "" : "+0.5 mp regen min +0.5 mp regen max ")); break;
		...
		case $stat[Moxie] : 		cli_execute("maximize "+max_bees+" beeosity, mainstat "+maxMainstat+" max, "+maxme+" -melee  " + ((bcasc_AllowML) ? "" : "-10 ml") + " +moxie experience +0.5 mp regen min +0.5 mp regen max "); break;
	}
	return true;
}
...
//Do we have enough cold resistance available?
int cold_res(int target, boolean do_stuff) {
	int coldi, colds, coldo;
	...
	if (colds >= target) {
		if (do_stuff) {
			...
			if (have_skill($skill[Scarysauce])) cli_execute("trigger lose_effect, Scarysauce, cast 1 Scarysauce");
	...
	foreach thing in stuff {
		if (numeric_modifier(thing, "Cold Resistance") > 0)
			coldi = coldi + numeric_modifier(thing, "Cold Resistance");
	}	
...
string consultHeBo(int round, string opp, string text) {
	...
	if (contains_text(text, "y hippy") || contains_text(text, "War Hippy") || contains_text(text, "Dwarf Foreman") || contains_text(text, "bobrace.gif") || contains_text(text, "Frat Warrior") || contains_text(text, "War Pledge") || isGremlin) {
		if (my_path() == "Bees Hate You") {
...
void defaultMood(boolean castMojo) {
	...
	if (have_skill($skill[Zombie Chow])) cli_execute("trigger lose_effect, Chow Downed, cast 1 Zombie Chow");
...
boolean can_adv(location where) {
	...
	case $location[Outskirts of Camp]: return (my_path() != "Zombie Slayer" && canadia_available());
...
boolean traverse_temple()
{
	...
	{
		if (have_effect($effect[stonefaced]) == 0)
	...
	if (my_path() != "Avatar of Boris") {
		if (contains_text(combat,"+")) {
			if (willMood()) {
				...
				if (have_skill($skill[Summon Horde])) cli_execute("trigger lose_effect, Waking the Dead, cast 1 Summon Minion");
		...
		if (contains_text(combat,"i")) {
			if (willMood()) {
				...
				//if (have_skill($skill[Zombie Chow]) && have_path_familiar($familiar[Reagnimated Gnome])) cli_execute("trigger lose_effect, Chow Downed, cast 1 Zombie Chow");
				//if (have_skill($skill[Scavenge])) cli_execute("trigger lose_effect, Scavengers Scavenging, cast 1 Scavenge");
...
location level_location(int value) {
	...
	if (value < 120) {
		...
		switch(my_primestat()) {
			case $stat[Muscle]: return $location[Haunted Gallery];
			case $stat[Mysticality]: return $location[Haunted Bathroom];
			case $stat[Moxie]: return $location[Haunted Ballroom];
		}
	}
	return best;
}
...
boolean bumAdv(location loc, string maxme, string famtype, string goals, string printme, string combat, string consultScript) {
	...
	if (consultScript == "consultRunaway" && bcasc_100familiar == "") {
		if (have_path_familiar($familiar[Stomping Boots]))
...
boolean bcascAirship() {
	...
	set_property("choiceAdventure182", "2");
...
void bcascBugbearHunt() {
	...
	if (to_int(get_property("biodataWasteProcessing")) < 3 && my_adventures() > 0)
		bumAdv($location[Sleazy Back Alley], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "", "+" + (3 - to_int(get_property("biodataWasteProcessing"))) + " BURT", "BCC: Hunting for Scavenger bugbears.", "+");
		
	if (to_int(get_property("biodataMedbay")) < 3 && my_adventures() > 0)
		bumAdv($location[Spooky Forest], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "",  "+" + (3 - to_int(get_property("biodataMedbay"))) + " BURT", "BCC: Hunting for Hypodermic bugbears.", "+");
		
	if (checkStage("bats1") && to_int(get_property("biodataSonar")) < 3 && my_adventures() > 0)
		bumAdv($location[Beanbat Chamber], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "",  "+" + (3 - to_int(get_property("biodataSonar"))) + " BURT", "BCC: Hunting for batbugbears.", "+");

	if (checkStage("knobking") && to_int(get_property("biodataScienceLab")) < 6 && my_adventures() > 0)
	...
	if (checkStage("cyrpt") && to_int(get_property("biodataMorgue")) < 6 && my_adventures() > 0)
	...
	if (is_equal_to(get_property("questL08Trapper"), "finished") && to_int(get_property("biodataSpecialOps")) < 6 && my_adventures() > 0)
		bumAdv($location[Ninja Snowmen], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "",  "+" + (6 - to_int(get_property("biodataSpecialOps"))) + " BURT", "BCC: Hunting for Black Ops Bugbears.", "");

	if (checkStage("airship") && to_int(get_property("biodataEngineering")) < 9 && my_adventures() > 0)
		bumAdv($location[Fantasy Airship], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "",  "+" + (9 - to_int(get_property("biodataEngineering"))) + " BURT", "BCC: Hunting for Battlesuit Bugbear Types.", "");
		
	if (checkStage("manorlibrary") && to_int(get_property("biodataNavigation")) < 9 && my_adventures() > 0 && my_basestat(my_primestat()) > 90)
		bumAdv($location[Haunted Gallery], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "",  "+" + (9 - to_int(get_property("biodataNavigation"))) + " BURT", "BCC: Hunting for ancient unspeakable bugbears.", "");		
	
	if (get_property("questL12War") == "finished" && to_int(get_property("biodataGalley")) < 9 && my_adventures() > 0) {
		if (bcasc_doWarAs == "frat")
			bumAdv($location[hippy camp (stone age)], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "",  "+" + (9 - to_int(get_property("biodataGalley"))) + " BURT", "BCC: Hunting for trendy bugbear chefs.", "");
		else
			bumAdv($location[frat house (stone age)], (i_a("bugbear detector") > 0 ? "+equip bugbear detector" : ""), "",  "+" + (9 - to_int(get_property("biodataGalley"))) + " BURT", "BCC: Hunting for trendy bugbear chefs.", "");
	}
}
...
boolean bcascCastle() {
	...
	if (in_hardcore() && my_path() != "Bugbear Invasion") {
...
boolean bcascChasm() {
	boolean needLowercaseN() {
		...
		if (!in_hardcore() || my_path() == "Bees Hate You" || my_path() == "Avatar of Boris" || my_path() == "Bugbear Invasion" || my_path() == "Zombie Slayer") return false;
		return true;
	}
...
boolean bcascEpicWeapons() {
	...
	if(my_path() != "way of the surprising fist") {
		...
		if (my_class() == $class[Seal Clubber] && my_basestat(my_primestat()) > 10 && i_a("Bjorn's Hammer") == 0 && i_a("Hammer of Smiting") == 0 && i_a("Sledgehammer of the V�lkyr") == 0) {
			if (dontHaveAstral())
...
boolean bcascFriarsSteel() {
	...
	if ((steelLimit() > 16 && my_path() != "Avatar of Boris" && my_path() != "Zombie Slayer") || (my_path() == "Avatar of Boris" && steelLimit() - to_int(have_skill($skill[Legendary Appetite])) * 5 > 21) || (my_path() == "Zombie Slayer" && steelLimit() - to_int(have_skill($skill[Insatiable Hunger])) * 5 > 21)) return true;
	if (i_a(steelName()) > 0) {
	...
	while (item_amount($item[Azazel's unicorn]) == 0) {
		...
		cli_execute("mood execute");
		buMax();
		levelMe(safeMox($location[Hey Deze Arena]), false);
	...
	while (item_amount($item[Azazel's lollipop]) == 0) {
		levelMe(safeMox($location[Belilafs Comedy Club]));
		...
		while (item_amount($item[observational glasses]) == 0) bumAdv($location[Belilafs Comedy Club], "", "items", "1 observational glasses, 5 imp air", "Getting the Observational Glasses", "i");
...
boolean bcascGuild() {
	if (!in_hardcore() || my_path() == "Avatar of Boris" || my_path() == "Zombie Slayer") return checkStage("guild", true);
					...
					switch (lairitems[x].a) {
						case "marzipan skull" :
							if (gnomads_available()) {
								...
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
							} else if (tryPull(lairitems[x].a, true))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
							else if (bumAdv($location[South of the Border], "", "items", "1 Meleegra pills", "Getting some Meelegra pills", "-i"))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
						break;
						...
						case " gremlin juice" :
							if (tryPull(lairitems[x].a))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
							else if (bumAdv($location[post-war junkyard], "", "hebo", "1 gremlin juice", "Getting gremlin juice", "i", "consultHeBo"))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
						break;
						...
						case "handsomeness potion" :
							if (tryPull(lairitems[x].a))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
							else if (bumAdv($location[South of the Border], "", "hebo", "1 handsomeness potion", "Getting a handsomeness potion", "+i"))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
						break;
						...
						case "thin black candle" :
							if (tryPull(lairitems[x].a))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
							else if (bumAdv($location[Giant's Castle], "", "items", "1 thin black candle", "Getting a thin black candle", "-i"))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
						break;
						...
						case "Mick's IcyVapoHotness Rub" :
							if (tryPull(lairitems[x].a))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
							else if(bumAdv($location[Giant's Castle], "", "items", "1 Mick's IcyVapoHotness Rub", "Getting a Mick's IcyVapoHotness Rub", "+i"))
								numGatesWeHaveItemFor = numGatesWeHaveItemFor + 1;
						break;
...
boolean bcascMacguffinPalindome() {
	...
	while (contains_text(visit_url("questlog.php?which=1"), "Never Odd")) {
		while (contains_text(visit_url("questlog.php?which=1"), "Palindome") && item_amount(to_item("i love me")) == 0) {
			...
			bumAdv($location[Palindome], "+equip talisman o' nam", (in_hardcore()) ? "hebo" : "", "1 stunt nuts, 1 I Love Me Vol I", "Getting the 'I Love Me' from the Palindome", "i", (in_hardcore()) ? "consultHeBo" : ""); 
		}
...
boolean bcascMacguffinPrelim() {
	...
	while (i_a("Talisman o' Nam") == 0) {
		...
		if (!contains_text(visit_url("cove.php"), "cove3_5x2b.gif")) {
			...
			bumAdv($location[Poop Deck], maxstring, "", "", "Opening Belowdecks", "-");
		}
...
void bcascNaughtySorceress() {
	if (contains_text(visit_url("main.php"), "lair.php")) {
		...
		while (!contains_text(visit_url("lair3.php"), "lair4.php")) {
			if (item_amount($item[hedge maze puzzle]) == 0) bumAdv($location[Hedge Maze], "", "items", "1 hedge maze puzzle", "Getting another Hedge Maze", "i");
			if (hedgemaze()) {}
		}
		...
		if (get_property("telescopeUpgrades") >= 1) {
			...
			for i from get_property("telescopeUpgrades").to_int() downto 1 {
				...
				if (i_a(level.thing) == 0) {
					if (((contains_text(level.section, "quest") ? get_property(level.section) : get_property("bcasc_stage_"+level.section)) == my_ascensions() || level.section == "") && (level.loc != "")) {
...
boolean bcascTrapper() {
	...
	if ((have_skill($skill[Musk of the Moose]) || have_skill($skill[Carlweather's Cantata of Confrontation]) || have_skill($skill[Summon Horde])) && cold_res(5, false) > 0) {
		if (is_not_yet(get_property("questL08Trapper"),"finished")) {
			if (i_a("ninja carabiner") == 0) {
				...
				while (i_a("ninja carabiner") == 0) {
					bumminiAdv(1, $location[Ninja Snowmen]);
				}
			}
		...
		if (is_not_yet(get_property("questL08Trapper"),"finished")) {
			...
			if (!contains_text(visit_url("place.php?whichplace=mclargehuge&action=trappercabin"), "spookin' them varmints")) {
				while (get_property("lastEncounter") != "3 eXXXtreme 4ever 6pack") {
					bumMiniAdv(1, $location[eXtreme Slope]);
				}
				print("BCC: Snowboarding up to the Icy peak and searching for Groar.", "purple");
				trapper = visit_url("place.php?whichplace=mclargehuge&action=trappercabin");
				//Unlock the Icy Peak
				visit_url("place.php?whichplace=mclargehuge&action=cloudypeak");
			}
		
			// TODO: FIX! - doesn't handle groar problems well.
			//Adventure at the Icy Peak
			for i from 1 to 4 {
				visit_url("place.php?whichplace=mclargehuge&action=cloudypeak2");
				bumRunCombat();
...
void bcs6() {
	...
	if(my_path() != "Bugbear Invasion") {
		...
		while ((i_a("eyepatch") == 0 || i_a("swashbuckling pants") == 0 || i_a("stuffed shoulder parrot") == 0 || i_a("flaregun") == 0) && i_a("pirate fledges") == 0) {
			bumAdv($location[Pirate Cove], "", "equipmentnc", "1 eyepatch, 1 swashbuckling pants, 1 stuffed shoulder parrot, 1 flaregun", "Getting the Swashbuckling Kit (and a flaregun)", "-i");
		}
	}
...
void bcs9() {
	...
	if (i_a("giant pinky ring") == 0 && get_dwelling() != $item[Frobozz]) { 
		cli_execute("leaflet");
		...
		switch (name) {
			...
			case "orchard" :
				...
				while (item_amount($item[heart of the filthworm queen]) == 0) {
					...
					if(my_path() != "Bees Hate You" && have_effect($effect[red tongue]) == 0) bumUse(1, $item[blue snowcone]);
					bumUse(2, $item[lavender candy heart]);
					bumUse(1, $item[resolution: be happier]);
