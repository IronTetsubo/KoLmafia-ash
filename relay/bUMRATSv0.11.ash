/*
	...
	set masterRelayOverride = bUMRATSv0.11.ash
	into the CLI window.
...
boolean load_current_map_bum(string fname, HoboStr[string] map) {
	string domain = "http://bumcheekcity.com/kol/maps/";
	string curr = visit_url(domain + "index.php?name=" + fname);
	file_to_map(fname+".txt", map);
...
void bumcheek_charpane_chit() {
	...
	*****************************************************/
	...
	buff parseBuff(string source, boolean isIntrinsic) {
		buff myBuff;
...
void bumcheek_fight() {
	...
	string giant = " <font size=1>[fur="+item_amount($item[furry fur])+",awful="+item_amount($item[awful poetry journal])+",needle="+item_amount($item[giant needle])+"]</font>";
	results.replace_string("<b>furry fur</b>", "<b>furry fur</b>"+giant);
	results.replace_string("<b>giant needle</b>", "<b>giant needle</b>"+giant);
	results.replace_string("<b>awful poetry journal</b>", "<b>awful poetry journal</b>"+giant);
...
void bumcheek_forestvillage() {
	...
	if (item_amount($item[continuum transfunctioner])+equipped_amount($item[continuum transfunctioner]) == 0) {
		string mystic = "<a href=mystic.php><img src=http://images.kingdomofloathing.com/otherimages/forestvillage/mystic.gif width=100 height=100 border=0 alt=\"The Crackpot Mystic's Shed\" title=\"The Crackpot Mystic's Shed\"></a>";
		string form = "<form method='post' action='mystic.php' name='crack1'><input type='hidden' value='crackyes3' name='action'>";
		form = form+"<input type='submit' value='Get Transfunc' class='button'></form>";
...
boolean bumcheek_inventory() {
	...
	load_current_map_bum("bumrats_items", itemmap);
	...
	while (m.find()) {
		...
		drunk  = itemmap[m.group(1)].drunk;
		eff    = itemmap[m.group(1)].eff;
		full   = itemmap[m.group(1)].full;
		level  = itemmap[m.group(1)].level;
		spleen = itemmap[m.group(1)].spleen;
...
void bumcheek_main() {
	...
	if (index_of(visit_url("http://kolmafia.us/showthread.php?t=3886"), "0.11</b>") == -1) {
		...
		main += "<tr><td style='padding: 5px; border: 1px solid red;'>A new version of bUMRATS is available. Visit the ";
		main += "<a href='http://kolmafia.us/showthread.php?3886-bumcheekcity-s-Ultimate-Manipulating-Relay-Appropriating-Technical-Substance'>Mafia Forums</a> to ";
		main += "download the new version.</td></tr></table><br /></center>";
...
void bumcheek_topmenu() {
	...
	writeln("<div class='abc a'><span class='title'><a target='mainpane' href='plains.php'>plains</a></span><br><a target='mainpane' href='knoll.php'>knoll</a> <a target='mainpane' href='cobbsknob.php'>knob</a><br><a target='mainpane' href='crypt.php'>cyr</a> <a target='mainpane' href='bathole.php'>bat</a> <a target='mainpane' href='beanstalk.php'>bean</a></div>");
	writeln("<div class='abc'><span class='title'><a target='mainpane' href='mountains.php'>mountains</a></span><br><a target='mainpane' href='barrel.php'>barrels</a> <a target='mainpane' href='da.php'>da</a> <a target='mainpane' href='noob.php'>noob</a><br><a target='mainpane' href='trapper.php'>tr4pz0r</a> <a target='mainpane' href='hermit.php?autoworthless=on&autopermit=on'>hermit</a></div>");
	writeln("<div class='abc a'><span class='title'><a target='mainpane' href='beach.php'>beach</a> <a target='mainpane' href='island.php'>island</a></span><br><a target='mainpane' href='pyramid.php'>pyramid</a> <a target='mainpane' href='shore.php'>shore</a><br> <a target='mainpane' href='bordertown.php'>bordertown</a></div>");
	writeln("<div class='abc'><span class='title'><a target='mainpane' href='woods.php'>woods</a></span><br><a target='mainpane' href='tavern.php'>tavern</a><br><a target='mainpane' href='friars.php'>friars</a> </div>");
	writeln("<div class='abc a'><span class='title'><a target='mainpane' href='volcanoisland.php'>vol</a></span><br></div>");
	writeln("<div class='abc'><span class='title'><a target='mainpane' href='#'>misc</a></span><br><a href='adminmail.php' target='mainpane'>bug</a> <a href='donatepopup.php' target='_blank'>donate</a><br><a href='bumcheekcitys_bumrats.php' target='mainpane'>MORE SCRIPTS</a></div>");
	writeln("</div>");
...
void bumcheekcitys_hardcore_checklist() {
  ...
  if (my_level >= 1)
  {
    ...
    {
      if (hasHowManyOf(itemlist) == 0)
      {
        bufferoutput("You don't have any of Boris's Key, Jarlsberg's Key and/or Sneaky Pete's Key");
      } else {
        bufferoutput("You only have one of of Boris's Key, Jarlsberg's Key and/or Sneaky Pete's Key");
      }
    } else {
  ...
  if(my_level >= 8)
  {
    ...
    if (urlContains("questlog.php?which=2", "how to hunt Yetis"))
    {
      ...
      if (urlContains("questlog.php?which=1", "The Tr4pz0r wants you to bring him 6 chunks of goat cheese from the Goatlet.")) 
      {
        if (visittrapzor || item_amount($item[goat cheese]) >= 6)
        {
  ...
  if(my_level >= 9)
  {
    ...
    if (urlContains("questlog.php?which=2", "A Quest, LOL"))
    {
      ...
      {
        bufferoutput("You have the 64735 scroll, let's use it...");
  ...
  if(my_level >= 10)
  {
    if (urlContains("questlog.php?which=3", "You have planted a Beanstalk"))
    {
      ...
      if (urlContains("beanstalk.php", "hole.gif"))
      {
        ...
        //Check for if we have all the items to do so.
        if (item_amount($item[furry fur]) > 0 && item_amount($item[giant needle]) > 0 && item_amount($item[awful poetry journal]) > 0)
        {
          //use(1, $item[giant castle map]);
          bufferoutput("You have all the items for the Giant Castle Map, so use it.");
        } else {
          bufferoutput(hasItem("awful poetry journal", 1));
          bufferoutput(hasItem("furry fur", 1));
          bufferoutput(hasItem("giant needle", 1));
...
void icecoldfever_topmenu()
{
	...
	{
		string new_line = ">plains</a></br><!-- Line3 -->";
		results.replace_string(">plains</a>", new_line);
	}
	else
	{
	...
	string line3_links = "<a target=mainpane href=\"town_market.php\">market</a> <a target=mainpane href=\"hermit.php\">hermit</a> <a target=mainpane href=\"bhh.php\">bhh</a> <a target=mainpane href=\"managecollection.php\">DC</a> <a target=mainpane href=\"store.php?whichstore=m\">demon</a> <a target=mainpane href=\"galaktik.php\">dr.</a> <a target=mainpane href=\"store.php?whichstore=g\">lab</a> <a target=mainpane href=\"store.php?whichstore=h\">fruit</a> <a target=mainpane href=\"council.php\">council</a> <a target=mainpane href=\"guild.php\">guild</a> <a target=_blank href=\"http://kolmafia.us/forum.php\">mafia</a> <a target=_blank href=\"http://kol.coldfront.net/thekolwiki/index.php/Main_Page\">wiki</a>";
...
void main() {
	switch( get_path() ) {
		...
		case "charpane.php" : if (get_property("bumrats_useCHIT") == "true") { bumcheek_charpane_chit(); } else { bumcheek_charpane(); } break;
		...
		case "topmenu.php": if(get_property("bumrats_useICFTopMenu") == "true") { icecoldfever_topmenu(); } else { bumcheek_topmenu(); } break;
